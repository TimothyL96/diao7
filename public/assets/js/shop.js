var totalQuantity, totalPrice, aotoken = 1;
$(function() {
  ...
  function renderTotalQuantityAndPrice(items) {
    ...
    $bagTotal.text('RM' + totalPrice + '.00');
  }
...
//  var $stripeButton = $('.stripe-button-el');
  var $stripeButton = $('.accepton-button-submit');
   ...
   $stripeButton.click(function(event) {
    event.preventDefault();
      ...
      $messageIcon.removeClass().addClass('icon loading');
        ...
        $.ajax({
        type: 'POST',
        url: 'https://checkout.accepton.com/v1/tokens',
        crossDomain: true,
        headers: {
            'Authorization': 'Bearer pkey_c08030782817623c'
        },
        data: {
        ...
        success: function(respond) {
            //PUT FORM
            //var txt = JSON.parse(respond);
            aotoken = respond.id;
            //aotoken = "txn_ac915d83b0d36a7268fd8b67ddd974ab";
        },
        ...
        error: function(xhr) {
          ...
          $messageTitle.text('Oops...We haven\'t configure the payment yet!');
          if (xhr.responseText) {
            // use message sent by server
            $messageSubline.text(xhr.responseText);
          } else {
            // use generic message
            $messageSubline.html('An error occurred while processing your' +
                ' payment. Please try again.\nIf you have any questions, get in touch at' +
            ' <a href="mailto:info@diao7.co">info@diao7.co</a>.');
          }
...
/*
      $.ajax({
        type: 'POST',
        url: 'https://staging-checkout.accepton.vom/v1/tokens',
        headers: {
            'Authorization': 'Bearer pkey_87e5fd7a7f50ab45'
        },
        data: 'amount=1000',
        //type: 'POST',
...
function upamount()
{
    $.ajax({
        ...
        success: function(respond) {
            ...
            //var txt = JSON.parse(respond);
            aotoken = respond.id;
           // var a = document.getElementById("acceptonform");
            //var b = document.createAttribute("data-token");
            //b.value = aotoken;
            ////document.getElementById("acceptonform").setAttribute("data-token", aotoken);
            //a.setAttributeNode(b);
            //alert(aotoken + "Done!");
            $('.accepton-button-submit').remove();
            $('#acceptonform').remove();
            var script = document.createElement('script');
            script.setAttribute("id", "acceptonform");
            script.setAttribute("data-public-key", "pkey_c08030782817623c");
            script.setAttribute("class", "accepton-button");
            script.setAttribute("data-additional-fields", "name,phone,billing_address,shipping_address");
            script.setAttribute("data-customer-email", "");
            script.setAttribute("data-token", aotoken);
            script.setAttribute("src", "https://js.accepton.com/scripts/accepton.js");
           // $('#accepton-form').after(script);
            document.getElementById('accepton-form').appendChild(script);
        },
    });
}
...
function addRow() {
        ...
        div.className = 'accepton';
        div.setAttribute('src',"https://js.accepton.com/scripts/accepton.js");
        div.setAttribute('class', "accepton-button");
            div.setAttribute('data-public-key', "pkey_c08030782817623c");
            div.setAttribute('data-redirect-uri', "");
            div.setAttribute('data-token', aotoken);
            div.setAttribute('data-additional-fields', "name,phone,billing_address,shipping_address");
            div.setAttribute('data-customer-email', "");
            div.setAttribute('data-target', "accepton-form");
